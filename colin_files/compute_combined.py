...
def computer(savedir,shapecut,dataloader,batchsize,model, tokenizer, device, printevery=1):
    ...
    with torch.no_grad():
        ...
        for d in tqdm(dataloader): 
            ...
            pselect=parr.cpu()[indx1,indx2,tok[:,1:]]
            codelength_t=-np.log2(pselect).numpy()

            #character level 
            preprobs=smat.mm(parr.permute(2,1,0).reshape(ncols,-1)).view(nrows,parr.shape[1],parr.shape[0]).permute(2,1,0) 
            probs=(preprobs/preprobs.sum(axis=-1,keepdim=True)).cpu()

            pleak=preprobs.sum(axis=-1).cpu().numpy()
            ent_c=-torch.sum(probs*torch.log2(probs+1e-10),axis=-1).numpy()
            
            charnext=lookup[tok]
            pselect=probs[indx1,indx2,charnext[:,1:]]
            codelength_c=-np.log2(pselect).numpy() 
            ...
            #save all data
            for fname,tensor in zip([aname,ename_t,ename_c,tname,cname_t, cname_c, lname, pname],[attn,ent_t, ent_c,tok,codelength_t, codelength_c, tlens, pleak]): #need to add oname 
